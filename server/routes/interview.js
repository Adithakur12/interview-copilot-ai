...
router.post('/resume/analyze', authenticate, async (req, res) => {
  ...
  // Save to DB
  const db = getDb();
  db.prepare(`INSERT INTO resumes (id, user_id, text, analysis, company) VALUES (?, ?, ?, ?, ?)`).run(
...
router.post('/resume/upload', authenticate, async (req, res) => {
  ...
  awardXP(req.user.id, 25, 'Resume upload');
...
router.post('/evaluate', authenticate, async (req, res) => {
  ...
  // Save to DB
  const db = getDb();
  db.prepare(`INSERT INTO interviews (id, user_id, company, type, question, answer, score, feedback, skills) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
    uuidv4(), req.user.id, company, 'mock', question, answer, finalResult.overallScore, finalResult.feedback, JSON.stringify([])
  );

  // Award XP
  awardXP(req.user.id, 50, 'Interview completed');
  if (finalResult.overallScore >= 80) awardXP(req.user.id, 30, 'High score bonus');
  if (finalResult.overallScore >= 100) awardXP(req.user.id, 100, 'Perfect score!');
...
router.post('/dsa-evaluate', authenticate, async (req, res) => {
  ...
  const db = getDb();
  db.prepare(`INSERT INTO interviews (id, user_id, type, question, answer, score, feedback) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(
    uuidv4(), req.user.id, 'dsa', question, answer, finalResult.score, finalResult.feedback
  );

  awardXP(req.user.id, 40, 'DSA practice completed');
...
router.post('/room/turn', authenticate, async (req, res) => {
  ...
  awardXP(req.user.id, 50, 'Voice room answer completed');
  if (evaluation.overallScore >= 80) awardXP(req.user.id, 30, 'High score bonus');
...
router.post('/room/review', authenticate, async (req, res) => {
  ...
  awardXP(req.user.id, 120, 'Completed voice interview room session');
  res.json(review);
});
...
router.get('/spaced-repetition', authenticate, (req, res) => {
  const db = getDb();
  const history = db.prepare('SELECT * FROM interviews WHERE user_id = ? AND type = ? ORDER BY created_at DESC LIMIT 20').all(req.user.id, 'dsa');
...
router.post('/spaced-repetition/complete', authenticate, (req, res) => {
  const { topic = 'arrays' } = req.body;
  awardXP(req.user.id, 35, `Spaced repetition: ${topic}`);
